...
def write_chain_stats_on_data():
    ...
    star_stats_path = os.path.join(star_path, f"{PREFIX}_star_stats.csv")
...
def init_files():
    ...
    in_out_stars_path = os.path.join(star_path, f"{PREFIX}_in_out_stars.csv")
    no_stars_path = os.path.join(star_path, f"{PREFIX}_no_star.csv")
    f_n_b_path = os.path.join(star_path, f"{PREFIX}_scammer_funder_and_beneficiary.csv")
    if not os.path.exists(in_stars_path):
        with open(in_stars_path, "w", newline='') as in_file:
            csv_writer = csv.writer(in_file, quotechar='"', delimiter='|', quoting=csv.QUOTE_ALL)
            csv_writer.writerow(pattern_header)
            in_file.close()

    if not os.path.exists(out_stars_path):
        with open(out_stars_path, "w", newline='') as out_file:
            csv_writer = csv.writer(out_file, quotechar='"', delimiter='|', quoting=csv.QUOTE_ALL)
            csv_writer.writerow(pattern_header)
            out_file.close()

    if not os.path.exists(in_out_stars_path):
        with open(in_out_stars_path, "w", newline='') as in_out_file:
            csv_writer = csv.writer(in_out_file, quotechar='"', delimiter='|', quoting=csv.QUOTE_ALL)
            csv_writer.writerow(pattern_header)
            in_out_file.close()

    if not os.path.exists(no_stars_path):
        with open(no_stars_path, "w", newline='') as no_stars_file:
            csv_writer = csv.writer(no_stars_file, quotechar='"', delimiter='|', quoting=csv.QUOTE_ALL)
            csv_writer.writerow(no_pattern_header)
            no_stars_file.close()
